...
CREATE FUNCTION Es100Porciento() 
...
AS  
	BEGIN 
		...
		SET XACT_ABORT ON 
			...
			SET @Id_Parentezco = (SELECT [Id_Parentezco] FROM [dbo].[Parentezco] WHERE [Nombre] = @inParentezco AND [activo]=1)
...
AS  
	BEGIN 
		...
		SET NOCOUNT ON 
		SET XACT_ABORT ON 
			DECLARE @Id_Beneficiario INT

			SET @Id_Beneficiario = @inId
...
AS  
	BEGIN 
		...
		SET XACT_ABORT ON 
			...
			SET @Id_Persona = (SELECT [Id_Persona] FROM [dbo].[Persona] WHERE [Nombre] = @inNombre_Persona AND [activo]=1)
			SET @Id_Cuenta = (SELECT [Id_Cuenta] FROM [dbo].[Cuenta] WHERE [Num_Cuenta] = @inNum_Cuenta AND [activo]=1)
			SET @Id_Parentezco = (SELECT [Id_Parentezco] FROM [dbo].[Parentezco] WHERE  [Nombre] = @Id_Parentezco AND [activo]=1)
			SET @Porcentaje = @inPorcentaje
	...
	BEGIN 
		...
		SET XACT_ABORT ON 
			BEGIN TRAN
				...
				SELECT Persona.Nombre, Cuenta.Num_Cuenta, Parentezco.Nombre, Porcentaje FROM Beneficiario,Persona,Cuenta,Parentezco 
				WHERE Beneficiario.Id_Cuenta = Cuenta.Id_Cuenta and Beneficiario.Id_Persona = Persona.Id_Persona and Beneficiario.Id_Parentezco = Parentezco.Id_Parentezco  
			COMMIT
		END TRY
		BEGIN CATCH;
			If @@TRANCOUNT > 0 
...
AS  
	BEGIN 
		...
		BEGIN CATCH;
			If @@TRANCOUNT > 0 
				ROLLBACK TRAN;
			THROW 82738,'Error: No se ha podido leer el beneficiario',1;
		END CATCH
	END
GO
